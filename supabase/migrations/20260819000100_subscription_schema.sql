-- 정기 구독 스키마 — **docs/supabase_subscription.sql 에서 승격(promote)**.
--
-- 왜 이 파일이 생겼나: user_subscriptions / subscription_webhook_events 와 두 RPC 는
-- 지금까지 docs/ 아래에만 있었다. supabase/migrations/ 에 없으므로 마이그레이션
-- 파이프라인으로는 **한 번도 적용되지 않는다**. 그런데 Phase 2 이후 이 테이블은
-- 프리미엄 생성 인가(premium_entitlement)의 근거가 됐다 — 없으면 구독 조회가
-- 실패하고, fail-closed 규칙에 따라 모든 프리미엄 생성이 503 으로 막힌다.
--
-- 내용은 원본과 동일하며 전부 멱등이다(create ... if not exists / or replace).
-- 이미 수동으로 적용한 환경에서 다시 돌려도 안전하다.
--
-- ⚠️ 원본(docs/supabase_subscription.sql)은 참조용으로 남긴다. 스키마를 바꿀 때는
--    **이 파일**을 고치고 docs 쪽을 맞춰야 한다.
